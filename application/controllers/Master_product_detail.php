...
class Master_product_detail extends CI_Controller {
    ...
    public function add_detail() 
    {
        ...
        $get_label = $this->Label_model->get_label_value($id,$row[0]->product_category_id);
        //echo count($get_label);die();
        ...
        //echo count($cekValue).'/';
        //echo '<pre>';print_r($cekLabel);die('xxx');
        if(empty($cekValue)){
    ...
    {       
        ...
        //echo $this->input->post('procat_id');        
    ...
    {             
        $data = array();
        $label = $this->Label_model->cek_label($this->input->post('id'), $this->input->post('procat_id'));    
        //echo $this->input->post('id').'/'.$this->input->post('procat_id').'<br>';
        //echo '<pre>';print_r($label);die();
        
        $this->Label_value_model->del_by_procat_id($this->input->post('procat_id'));

        foreach ($label as $p) {
            //$data['label_name'] = $p->label_name;
            //$data['label_value'] = $this->input->post($p->label_name);               
            $this->Label_value_model->update_label_value($this->input->post('id'),$this->input->post('procat_id'),$p->label_name, $this->input->post($p->label_name));       
            //echo $this->input->post('id').'/'.$this->input->post('procat_id').'/'.$p->label_name.'/'. $this->input->post($p->label_name).'<br>';
        } 
        //die('stop');           
